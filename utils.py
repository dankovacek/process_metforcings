...
def extract_info_from_url(url: str) -> tuple[str, str, str]:
    """
    Dummy parser for URL.
    Modify this function based on your URL structure so that it returns:
    (param, tile_no, yyyy)
    
    For example, if the URL is:
      http://example.com/data/temperature/12093/1980/dayl.nc
    You might extract:
      param   = "temperature"
      tile_no = "12093"
      yyyy    = "1980"
    """
    parts = url.split('/')[7].split('_')
    param = parts[4]
    yyyy = parts[5].split('.')[0]
    tile_no = 1111
    # Adjust indices as needed. Here we assume:
    #   parts[-4] is <param>
    #   parts[-3] is <tile_no>
    #   parts[-2] is <yyyy>
    # param = parts[-4]
...
def clip_dataset(
    ...
    try:
        ...
        if isinstance(geometry, Polygon):
            ds = ds.rio.clip([geom], all_touched=True, drop=True)
